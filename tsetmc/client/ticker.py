...
class Ticker:
    def __init__(self, **kw):
        self.__symbol_to_index_map = None

        if 'symbol' in kw:
            self.symbol = kw['symbol']
            self.index = StockController.get_index(self.symbol)
        elif 'index' in kw:
            self.index = kw['index']
            self.symbol = StockController.get_symbol(self.index)

        details = StockController.get_details(symbol=self.symbol)
        ...
        self._price_csv_path = os.path.join(settings.STOCKS_PRICE_DATA_PATH, f'{self.symbol}.csv')
        self._client_type_csv_path = os.path.join(settings.STOCKS_CLIENT_TYPE_DATA_PATH, f'{self.symbol}.csv')
        self._shareholders_csv_path = os.path.join(settings.STOCKS_SHAREHOLDERS_DATA_PATH, f'{self.symbol}.csv')
